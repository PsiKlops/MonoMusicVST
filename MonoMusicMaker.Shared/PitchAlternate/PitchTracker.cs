...
namespace Pitch
{
    ...
    public class PitchTracker
    {
        ...
        private float mPosConvertSec = 0f;
        private float m_detectLevelThreshold = 0.01f;       // -40dB
        public const int PITCH_RECORDS_PER_SEC = 50;
        public const float TIME_PER_PITCH_RECORD = 1f/ PITCH_RECORDS_PER_SEC;
        public int m_pitchRecordsPerSecond = PITCH_RECORDS_PER_SEC;           // default is 50, or one record every 20ms
        ...
        public double SampleRate
        {
            set
            {
                ...
                m_sampleRate = value;
                mPosConvertSec = 1f / (float)m_sampleRate;
                Setup();
            }
        }
        ...
        public void ProcessBuffer(float[] inBuffer, TimeSpan ct, int sampleCount = 0)
        {
            if (inBuffer == null)
            ...
            while (samplesProcessed < srcLength)
            {
                ...
                while (m_circularBufferLo.ReadBuffer(m_pitchBufLo, m_curPitchSamplePos, m_pitchBufSize + m_detectOverlapSamples))
                {
                    ...
                    AddPitchRecord(detectedPitch, ct);
        ...
        private void AddPitchRecord(float pitch, TimeSpan ct)
        {
            var midiNote = 0;
            var midiCents = 0;
            ...
            record.mTimeWhen = ct;
            record.m_curPitchSamplePos = (int)m_curPitchSamplePos;
            record.mTimeFromPos = mPosConvertSec * m_curPitchSamplePos;
        ...
        public struct PitchRecord
        {
            ...
            public TimeSpan mTimeWhen;
            public int m_curPitchSamplePos;
            public float mTimeFromPos;
        }
    }
}
